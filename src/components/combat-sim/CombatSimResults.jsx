...
export default function CombatSimResults({ battleLog, winner, combatants, onReset, colors }) {
  ...
  const dmgA = rounds.filter(r => r.attacker === combatants.a?.name && r.hit).reduce((s, r) => s + (r.damage || 0), 0);
  const dmgB = rounds.filter(r => r.attacker === combatants.b?.name && r.hit).reduce((s, r) => s + (r.damage || 0), 0);
  const critsA = rounds.filter(r => r.attacker === combatants.a?.name && r.critical).length;
  const critsB = rounds.filter(r => r.attacker === combatants.b?.name && r.critical).length;
  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        className="text-center rounded-lg p-6" style={{ background: accentA + '15', border: `1px solid ${accentA}50` }}>
        ...
        {conclusion?.narration && <p className="text-sm italic mt-3 max-w-md mx-auto" style={{ color: text1 }}>"{conclusion.narration}"</p>}
      </motion.div>

      {/* MVP moment */}
      {conclusion?.mvp_moment && (
        <div className="rounded-lg p-4 flex items-start gap-3" style={{ background: panel0, border: `1px solid #FFD70040` }}>
          <Star className="h-4 w-4 flex-shrink-0 mt-0.5" style={{ color: '#FFD700' }} />
          <div>
            <div className="text-[9px] font-mono uppercase tracking-widest mb-1" style={{ color: '#FFD700' }}>MVP MOMENT</div>
            <p className="text-xs" style={{ color: text1 }}>{conclusion.mvp_moment}</p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-lg p-4" style={{ background: panel0, border: `1px solid ${accentA}20` }}>
        ...
        <div className="grid grid-cols-3 gap-1 text-center text-xs">
          <div style={{ color: accentA }} className="font-mono font-bold text-[11px] truncate">{combatants.a?.name}</div>
          <div style={{ color: muted }} className="font-mono uppercase text-[9px] self-end pb-0.5">STAT</div>
          <div style={{ color: '#FF6B6B' }} className="font-mono font-bold text-[11px] truncate">{combatants.b?.name}</div>
          {[
            [hitsA, 'Hits', hitsB],
            [dmgA, 'Damage', dmgB],
            [critsA, 'Crits', critsB],
            ...
            <React.Fragment key={label}>
              <div className="py-1.5 rounded my-0.5" style={{ background: panel1, color: text0 }}>{valA}</div>
              <div className="py-1.5 self-center my-0.5 text-[10px]" style={{ color: muted }}>{label}</div>
              <div className="py-1.5 rounded my-0.5" style={{ background: panel1, color: text0 }}>{valB}</div>
            </React.Fragment>
          ))}
        </div>
      </div>
      ...
      <details>
        <summary className="cursor-pointer text-xs font-mono uppercase tracking-wide py-2 px-3 rounded" style={{ background: panel0, color: muted }}>
          Full Combat Log ({rounds.length} rounds)
        </summary>
        <div className="mt-2 space-y-1 max-h-64 overflow-y-auto">
          {rounds.map((line, i) => (
            <div key={i} className="p-2 rounded text-xs" style={{ background: panel1, border: line.critical ? `1px solid #FFD70040` : 'none' }}>
              <span className="font-mono text-[9px]" style={{ color: muted }}>RND{line.round} </span>
              {line.critical && <span className="text-[9px] font-mono font-black px-1 rounded mr-1" style={{ background: '#FFD700', color: '#000' }}>CRIT</span>}
              ...
              {line.status && <span className="ml-1 text-[9px] font-mono px-1 rounded" style={{ background: '#A855F720', color: '#A855F7' }}>{line.status}</span>}
              {line.env_trigger && <span className="ml-1 text-[9px]" style={{ color: '#FFC857' }}>⚡{line.env_trigger}</span>}
            </div>
          ))}
        </div>
      </details>
